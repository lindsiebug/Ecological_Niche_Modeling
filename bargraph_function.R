bar2 <- function(dv, factors, dataframe, percentage=FALSE, errbar=!percentage, half.errbar=TRUE, conf.level=.95, 
  ...
  if(legend & !percentage){
    ...
    args.legend.temp<-list(x="topleft", bty=if(!legend.border)"n" else "o",
                           inset=c(0,0))
